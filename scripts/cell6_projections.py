...
# fetches book odds (BDL first, Odds API fallback),
# saves to Google Sheets Projections tab.
#
...
# ── LOAD MODELS ──
MODEL_PATH = os.path.join(os.path.dirname(__file__), "models", "game_models.pkl")
with open(MODEL_PATH, "rb") as f:
    _art = pickle.load(f)

scaler       = _art["scaler"]
model_home   = _art["model_home"]
model_away   = _art["model_away"]
model_total  = _art["model_total"]
model_win    = _art["model_win"]
...
# ── STEP 3 — ODDS (BDL primary, Odds API fallback) ──
...
def get_odds_api_lines(home_abbr, away_abbr):
    r = requests.get(
        f"{ODDS_API_BASE}/sports/basketball_wnba/odds",
        params={
            "apiKey":    ODDS_API_KEY,
            "regions":   "us",
...
for game in todays_games:
    ...
    odds = get_game_odds(game_id, home["abbreviation"], away["abbreviation"])
    time.sleep(0.1)
...
print(f"\nSaved {len(df_proj)} projection(s) to Projections tab.")
print("Run cell7_signals.py for game signals.")
